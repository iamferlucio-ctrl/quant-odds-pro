...
<style>
    ...
    .forecast-sub { font-size: 0.75rem; color: #10B981; font-weight: 600; }
...
def analyze_match_complete(
    ...
    forecast = DirectionalForecast(
        favorite_team=fav_team, underdog_team=und_team, expected_winner=exp_winner,
        winner_confidence=round(win_conf * 100, 1), expected_goals_trend=goals_trend,
        goals_confidence=round(goals_conf * 100, 1), handicap_forecast=handicap_forecast,
        most_probable_score=best_score, score_probability=round(best_score_prob * 100, 1)
    )
    ...
    if best_ev < min_ev_threshold:
        is_approved = False
        warnings.append(f"Mercado Eficiente: EV ({best_ev*100:+.1f}%) no alcanza el umbral de +{min_ev_threshold*100:.1f}%.")

    if ev_1x2_home > 0.035 and ev_ah < -0.01:
        is_approved = False
        warnings.append("🚨 Trampa de Liquidez: Discrepancia entre cuota 1X2 e Hándicap Asiático.")

    if abs(imp_lh - f_lh) > 0.55:
        is_approved = False
        warnings.append(f"🚨 Divergencia xG: Casa ({imp_lh}) vs Modelo ({f_lh}).")
    ...
    # CORRECCIÓN DE CÁLCULO KELLY EN DÓLARES:
    ...
    if is_approved:
        ...
        narrative = f"Ventaja en **{best_market_name}** con **EV de {best_ev*100:+.2f}%**, alineado con la dirección prevista."
    else:
        action_text = "ABSTENERSE / MERCADO EFICIENTE"
        narrative = f"Favorito **{exp_winner}**, pero la cuota ({best_odds}) ya refleja el riesgo del mercado."
...
def_home, def_away = "LDU Quito", "Mirassol"
...
<div class="match-header">
    ...
    <div class="match-subtitle">ANÁLISIS DIRECCIONAL DE CONSENSO Y EJECUCIÓN</div>
</div>
""", unsafe_allow_html=True)

# 2. PRONÓSTICO DIRECCIONAL
st.markdown(f"""
<div class="forecast-container">
    <div class="forecast-title">🔮 PRONÓSTICO DIRECCIONAL DE MERCADO</div>
...
<div class="ticket-card {ticket_cls}">
    ...
    <div class="exec-grid">
        <div><div class="exec-item-label">Selección Recomendada</div><div class="exec-item-val">{order.target_selection if order.is_approved else 'Ninguna'}</div></div>
...
# 5. GRÁFICOS OPTIMIZADOS PARA MÓVIL (SIN ZOOM TÁCTIL)
